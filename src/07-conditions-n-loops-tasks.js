...
function doRectanglesOverlap(rect1, rect2) {
  const rect1Horizontal = getIntervalArray(rect1.left, rect1.left + rect1.width);
  const rect2Horizontal = getIntervalArray(rect2.left, rect2.left + rect2.width);
  const resultHorizontal = rect1Horizontal.some((el) => rect2Horizontal.indexOf(el) >= 0);
  const rect1Vertical = getIntervalArray(rect1.top, rect1.top + rect1.height);
  const rect2Vertical = getIntervalArray(rect2.top, rect2.top + rect2.height);
  const resultVertical = rect1Vertical.some((el) => rect2Vertical.indexOf(el) >= 0);
  if (resultHorizontal && resultVertical) return true;
  return false;
}
...
function isInsideCircle(circle, point) {
  ...
  const xy = Math.sqrt(x + y);
  return (xy < circle.radius);
}
...
function findFirstSingleChar(str) {
  const result = str.split('').sort().join('').replace(/([A-Za-z])\1+/g, '')
    .trim()[0];
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${left}${a > b ? b : a}, ${a > b ? a : b}${right}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < bracketsPairs.length; j += 1) {
...
function getCommonDirectoryPath(pathes) {
  ...
  if (pathes.every((el) => el[0] === '/')) {
    const arr = pathes.sort((a, b) => a.length - b.length).map((el) => el.split('/'));
    arr[0].forEach((el, ndx) => {
      const item = el;
      if (arr.every((el2) => el2[ndx] === item)) {
        resultArr.push(item);
      }
    });
